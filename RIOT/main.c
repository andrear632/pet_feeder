// Local broker:
// in ~/Desktop/mosquitto.rsmb/rsmb launch ./src/broker_mqtts config.conf
//
// Transparent bridge:
// in ~/Desktop/Individuale launch python3 transparent_bridge.py
...
void dispense(void){ //stepper motor
    if (can_dispense) {
        can_dispense = 0;
        int steps = 0;
        int count = 2000; //controlla se valore va bene
        while (count) {
            switch(steps) {
                case 0:
...
int read_distance(void){ //ultrasonic sensor
	...
	xtimer_usleep(20); //at least 10 from datasheet
	gpio_set(trigger_pin);
	xtimer_msleep(100); //wait callback function to act
	return echo_time;
}
